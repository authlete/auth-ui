    ...
    >
      <div className={cn(hideNav && "hidden")}>
        <TabsList aria-label={localization.settings.settings}>
          <TabsTrigger nativeButton={false} value="account" render={<Link href={`${basePaths.settings}/${viewPaths.settings.account}`} className="gap-1" />}><User2 className="text-muted-foreground" />{localization.settings.account}</TabsTrigger>

          <TabsTrigger nativeButton={false} value="security" render={<Link href={`${basePaths.settings}/${viewPaths.settings.security}`} className="gap-1" />}><Shield className="text-muted-foreground" />{localization.settings.security}</TabsTrigger>

          {plugins.flatMap(
            (plugin) =>
              plugin.settingsTabs?.map((settingsTab, index) => (
                <TabsTrigger nativeButton={false} key={`${plugin.id}-${index.toString()}`} value={settingsTab.view} render={<Link href={`${basePaths.settings}/${plugin.viewPaths?.settings?.[settingsTab.view]}`} className="gap-1" />}>{settingsTab.label}</TabsTrigger>
              )) ?? []
          )}
        </TabsList>
      </div>
